defmodule PortalDeployment.GameFileSystem.ModOverridesLua do
  ...
  def path(cluster_id, shard_id),
    do: ShardFolder.path(cluster_id, shard_id) <> "/modoverrides.lua"
  ...
  def parse_file(contents) do
    {data_map, nil} =
      ...
      |> Enum.reduce({%{}, nil}, fn
        ...
        "  [\"workshop-" <> rest, {data_map, nil} ->
          {data_map, rest |> String.split("\"") |> Enum.at(0)}

        # Start recording config options
        "    configuration_options = {", {data_map, key} ->
          {data_map, key, ""}

        # Stop recording config options
        "    }", {data_map, key, text} ->
          {Map.put(data_map, key, text), nil}
