defmodule PortalDeployment.Configuration.WorldGenStorage do
  ...
  def get!(cluster_id, shard_id) do
    ...
    |> Map.update("ocean_seastack", "default", fn value -> (value |> String.split("_") |> Enum.at(1)) end)
    |> Map.update("ocean_waterplant", "default", fn value -> (value |> String.split("_") |> Enum.at(1)) end)
  end
  ...
  defp stringify_gen(gen) do
    ...
    |> Enum.reduce(%{}, fn
      ...
      {:ocean_seastack, :default}, acc -> Map.put(acc, "ocean_seastack", "default")
      {:ocean_seastack, value}, acc -> Map.put(acc, "ocean_seastack", "ocean_" <> Atom.to_string(value))
      {:ocean_waterplant, :default}, acc -> Map.put(acc, "ocean_waterplant", "default")
      {:ocean_waterplant, value}, acc -> Map.put(acc, "ocean_waterplant", "ocean_" <> Atom.to_string(value))
